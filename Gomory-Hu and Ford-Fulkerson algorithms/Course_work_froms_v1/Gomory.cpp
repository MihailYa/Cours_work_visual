...
namespace coursework
{
	...
	void recurs_Gomory(graph *&gr_result, graph **&graphs, int ver, FILE **f_output)
	{
		...
		graph *gr_first = condense(graphs[ver], 0, vect, gr_result->n_vertexes);
		...
		graph *gr_second = condense(graphs[ver], 1, vect, gr_result->n_vertexes);
		...
		{
			// Set on his "ver" place second half
	...
	void cg_adj(graph *&gr_result, graph **graphs, int ver, int *vect)
	{
		...
		for (int i = 0; i < gr_result->n_vertexes; i++)
		{
			...
			// If union_vertex not found in graphs[ver]
